...
public class FS20Binding extends AbstractActiveBinding<FS20BindingProvider>
	...
	if (bindingConfig != null) {
	    ...
	    Command newCommand = FS20Shutter.handleCommand(itemName, command,
		    eventPublisher);
	    if (newCommand == null) {
		logger.debug("filtered command " + command.toString()
			+ " for item " + itemName);
	    } else {
		try {
		    // Map command
		    FS20Command fs20Command = FS20CommandHelper
			    .convertHABCommandToFS20Command(newCommand);
		    cul.send("F" + bindingConfig.getAddress()
			    + fs20Command.getHexValue());
		} catch (CULCommunicationException e) {
		    logger.error(
			    "An exception occured while sending a command", e);
		}
	    }
	}
    }
	...
	if (config != null) {
	    ...
	    /*
	     * PARITY_EVEN 2 PARITY_MARK 3 PARITY_NONE 0 PARITY_ODD 1
	     * PARITY_SPACE 4
	     */
