...
/**
 * <p>
 * This class acts as a REST resource for sitemaps and provides different
 * methods to interact with them, like retrieving a list of all available
 * sitemaps or just getting the widgets of a single page.
 * </p>
 * 
 * <p>
...
public class SitemapResource {
    ...
    private static final Logger logger = LoggerFactory
	    .getLogger(SitemapResource.class);
	...
	if (responseType != null) {
	    ...
	    // check for forwarded scheme
	    java.util.List<String> forwardedSchemes = headers
		    .getRequestHeader("X-Forwarded-Scheme");
	    if (!forwardedSchemes.isEmpty()) {
		String forwardedScheme = forwardedSchemes.get(0); // assume one!

		// scheme:]scheme-specific-part[#fragment]
		try {
		    uri = new URI(forwardedScheme,
			    uri.getRawSchemeSpecificPart(),
			    uri.getRawFragment());
		} catch (URISyntaxException e) {
		    logger.warn("cant happen", e);
		}
	    }

	    Object responseObject = responseType
		    .equals(MediaTypeHelper.APPLICATION_X_JAVASCRIPT) ? new JSONWithPadding(
		    new SitemapListBean(getSitemapBeans(uri)), callback)
		    : new SitemapListBean(getSitemapBeans(uri));
	...
	if (responseType != null) {
	    Object responseObject = responseType
		    .equals(MediaTypeHelper.APPLICATION_X_JAVASCRIPT) ? new JSONWithPadding(
		    getSitemapBean(sitemapname, uriInfo.getBaseUriBuilder()
	...
	if (atmosphereTransport == null || atmosphereTransport.isEmpty()) {
		...
		Object responseObject = responseType
			.equals(MediaTypeHelper.APPLICATION_X_JAVASCRIPT) ? new JSONWithPadding(
			getPageBean(sitemapname, pageId, uriInfo
				.getBaseUriBuilder().build()), callback)
			: getPageBean(sitemapname, pageId, uriInfo
				.getBaseUriBuilder().build());
		throw new WebApplicationException(
			Response.ok(responseObject, responseType)
				.header(ATMOS_TIMEOUT_HEADER,
					DEFAULT_TIMEOUT_SECS + "").build());
	    } else {
	...
	GeneralBroadcaster sitemapBroadcaster = BroadcasterFactory.getDefault()
		.lookup(GeneralBroadcaster.class,
			resource.getRequest().getPathInfo(), true);
	sitemapBroadcaster
		.addStateChangeListener(new SitemapStateChangeListener());
	...
	return new SuspendResponse.SuspendResponseBuilder<Response>()
		.scope(SCOPE.REQUEST).resumeOnBroadcast(resume)
		.broadcaster(sitemapBroadcaster)
		.period(DEFAULT_TIMEOUT_SECS, TimeUnit.SECONDS)
		.outputComments(true).build();
    }

    static public PageBean getPageBean(String sitemapName, String pageId,
	    URI uri) {
	ItemUIRegistry itemUIRegistry = RESTApplication.getItemUIRegistry();
	Sitemap sitemap = getSitemap(sitemapName);
	if (sitemap != null) {
	    if (pageId.equals(sitemap.getName())) {
		return createPageBean(sitemapName, sitemap.getLabel(),
			sitemap.getIcon(), sitemap.getName(),
			sitemap.getChildren(), false,
			isLeaf(sitemap.getChildren()), uri);
	    } else {
		Widget pageWidget = itemUIRegistry.getWidget(sitemap, pageId);
		if (pageWidget instanceof LinkableWidget) {
		    EList<Widget> children = itemUIRegistry
			    .getChildren((LinkableWidget) pageWidget);
		    PageBean pageBean = createPageBean(sitemapName,
			    itemUIRegistry.getLabel(pageWidget),
			    itemUIRegistry.getIcon(pageWidget), pageId,
			    children, false, isLeaf(children), uri);
			...
			String parentId = itemUIRegistry
				.getWidgetId((Widget) parentPage);
			...
			pageBean.parent = getPageBean(sitemapName,
				sitemap.getName(), uri);
			pageBean.parent.widgets = null;
		    }
		    return pageBean;
			...
			if (pageWidget == null) {
			    logger.debug(
				    "Received HTTP GET request at '{}' for the unknown page id '{}'.",
				    uri, pageId);
			} else {
			    logger.debug(
	    ...
	    logger.info(
		    "Received HTTP GET request at '{}' for the unknown sitemap '{}'.",
		    uri, sitemapName);
	    throw new WebApplicationException(404);
	}
    }
	...
	for (String modelName : modelRepository
		.getAllModelNamesOfType("sitemap")) {
		...
		bean.link = UriBuilder.fromUri(uri).path(bean.name).build()
			.toASCIIString();
	...
	bean.link = UriBuilder.fromUri(uri).path(SitemapResource.PATH_SITEMAPS)
		.path(bean.name).build().toASCIIString();
	bean.homepage = createPageBean(sitemap.getName(), sitemap.getLabel(),
		sitemap.getIcon(), sitemap.getName(), sitemap.getChildren(),
		true, false, uri);
	return bean;
    }

    static private PageBean createPageBean(String sitemapName, String title,
	    String icon, String pageId, EList<Widget> children,
	...
	bean.icon = icon;
	bean.leaf = isLeaf;
	bean.link = UriBuilder.fromUri(uri).path(PATH_SITEMAPS)
		.path(sitemapName).path(pageId).build().toASCIIString();
	if (children != null) {
	    int cntWidget = 0;
	    for (Widget widget : children) {
		String widgetId = pageId + "_" + cntWidget;
		WidgetBean subWidget = createWidgetBean(sitemapName, widget,
			drillDown, uri, widgetId);
		if (subWidget != null)
    ...
    static private WidgetBean createWidgetBean(String sitemapName,
	    Widget widget, boolean drillDown, URI uri, String widgetId) {
	ItemUIRegistry itemUIRegistry = RESTApplication.getItemUIRegistry();
	...
	if (widget.getItem() != null) {
	    ...
	    if (item != null) {
		bean.item = ItemResource.createItemBean(item, false, UriBuilder
			.fromUri(uri).build().toASCIIString());
	    }
	}
	...
	if (widget instanceof LinkableWidget) {
		...
		for (Widget child : children) {
		    ...
		    WidgetBean subWidget = createWidgetBean(sitemapName, child,
			    drillDown, uri, widgetId);
		    if (subWidget != null) {
			bean.widgets.add(subWidget);
			cntWidget++;
		...
		bean.linkedPage = createPageBean(sitemapName,
			itemUIRegistry.getLabel(widget),
			itemUIRegistry.getIcon(widget), pageName,
			drillDown ? children : null, drillDown,
			isLeaf(children), uri);
	    }
	}
	if (widget instanceof Switch) {
		...
		if (mapping.getCmd() != null) {
		    if (mapping.getCmd().startsWith("\"")
			    && mapping.getCmd().endsWith("\"")) {
			mappingBean.command = mapping.getCmd().substring(1,
				mapping.getCmd().length() - 1);
		    } else {
	...
	if (widget instanceof Image || widget instanceof Video
	    ...
	    sbBaseUrl.append(uri.getScheme()).append("://")
		    .append(uri.getHost());
		...
		try {
		    sb.append("&")
			    .append("baseUrl=")
			    .append(URLEncoder.encode(sbBaseUrl.toString(),
				    "UTF-8"));
	...
	for (Widget w : children) {
		...
		ItemUIRegistry itemUIRegistry = RESTApplication
			.getItemUIRegistry();
	...
	if (repo != null) {
	    Sitemap sitemap = (Sitemap) repo.getModel(sitemapname
		    + SITEMAP_FILEEXT);
	    return sitemap;
	}
	return null;
