...
public class HueActions {
    ...
    private static final Logger logger = LoggerFactory
	    .getLogger(HueActions.class);

    // /**
    // * set Group to those id's; if a group with this name doesn't exist,
    // create a new own
	...
	try {
		...
		result = resourceRequest(HttpMethod.PUT, "scenes/" + sceneId
			+ "/lights/" + light + "/state", body);
	    }
	} catch (HueResourceRequestException e) {
	    logger.warn("hueDeleteAllRules failed", e);
	    result = e.getResponses().toString();
	}
	return result;
    }

    /**
     * Create a new Scene on the bridge, and store all lights' settings for this
     * scene
     * 
     * @param sceneId
     *            Id of the Scene
	...
	String result;
	try {
	    result = resourceRequest(HttpMethod.PUT, "scenes/" + sceneId, s);
	} catch (HueResourceRequestException e) {
	    logger.warn("hueDeleteAllRules failed", e);
	    result = e.getResponses().toString();
	}
    ...
    public static String hueAddGroupAction(
	    ...
	    @ParamDoc(name = "bodyValue", text = "Value of the element.") Object bodyValue)
	    throws IOException {
    ...
    public static void hueSetSceneForButton(
	    ...
	    @ParamDoc(name = "sensorId", text = "Id of tap device") String sensorId,
	    @ParamDoc(name = "button", text = "Button id 1..4") int button,
	    @ParamDoc(name = "scene", text = "scene id") String scene)
	    throws IOException {
	...
	try {
		...
		resourceRequest(HttpMethod.DELETE, "rules/" + ruleId,
			(String) null);
	    }
	} catch (HueResourceRequestException e) {
	    logger.warn("hueDeleteAllRules failed", e);
    ...
    protected static HueSettings getHueSettings() {
    ...
    /**
     * create a List<String> from id's seperated by , and blanks; ignore empty
     * ones
     * 
     * @param ids
     * @return
     ...
     * @param path
     * @return
     * @throws HueResourceRequestException
     */
     ...
     * @throws HueResourceRequestException
     */
    protected static String putResource(String path, String body)
	    throws HueResourceRequestException {
     ...
     * @throws HueResourceRequestException
     */
    protected static String postResource(String path, String body)
	    throws HueResourceRequestException {
    ...
    protected static String resourceRequest(HttpMethod method, String path,
	    AbstractHueResource resource) throws HueResourceRequestException {
	String body = null;
	if (resource != null) {
	    try {
     ...
     * @throws HueResourceRequestException
     */
    private static String resourceRequest(HttpMethod method, String path,
	    String body) throws HueResourceRequestException {
	...
	try {
		...
		if (err) {
		    throw new HueResourceRequestException(
			    "failed to send request '" + path + "'", responses);
		}
	    } catch (IOException e) {
    ...
    // * @return
    // */
    // private static boolean sendMessage(String url, String body, String
    // options) {
    ...
    // } else {
    // try {
    // context.getHomematicClient().setRemoteControlDisplay(remoteControlAddress,
    // text, options);
    // return true;
